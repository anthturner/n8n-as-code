...
`;
  ...
  private getAgentsContent(n8nVersion: string): string {
    return [
      ...
      `- **CRITICAL (TYPE)**: The \`type\` field MUST EXACTLY match the \`type\` from the schema (e.g., \`n8n-nodes-base.switch\`, NOT just \`switch\`).`,
      `- **CRITICAL (VERSION)**: The \`typeVersion\` MUST ALWAYS be the HIGHEST number from the \`version\` array in the schema (e.g., if version is [1, 2, 3], use \`3\`).`,
      `- **PARAMETER NAMES**: Use exact parameter names (e.g., \`spreadsheetId\` vs \`spreadsheet_id\`).`,
      `- **NO HALLUCINATIONS**: Do not invent parameter names or assume parameters from your training data.`,
      ``,
      `### Step 4: Validate Before Committing`,
      `- Double-check that \`type\` includes the package prefix (e.g., \`n8n-nodes-base.\`).`,
      `- Verify \`typeVersion\` is matches the latest extracted version.`,
      ...
      `| **Full Type** | \`"type": "n8n-nodes-langchain.agent"\` | \`"type": "agent"\` |`,
      `| **Version** | \`"typeVersion": 3.4\` (if 3.4 is latest) | \`"typeVersion": 1\` (outdated) |`,
      ``,
      `> [!IMPORTANT]`,
      `> n8n will display a **"?" (question mark)** if you forget the package prefix (\`n8n-nodes-base.\`). Always include it!`,
      ``,
      `---`,
      ``,
      `## ✅ Best Practices`,
